...
public final class Path implements Comparable<Path>, Serializable {
  ...
  public static Path commonPart(Path p1, Path p2) {
    int c1 = p1.getElementCount();
    int c2 = p2.getElementCount();
  ...
  /**
   * Returns this path object encoded as a link: if the path is not empty, adds
  ...
  public Path getPartial(int count) {
    ...
    System.arraycopy(elements, 0, items, 0, count);
   ...
   *         given path directly. Example:    <pre>
   *                 Path myPath = new Path(&quot;home/user/myfile.txt&quot;);
   ...
   *
   * @return <code>true</code> if the current path is contained in the given
   *         path (at any depth). Example:    <pre>
   *                 Path myPath = new Path(&quot;home/user/myfile.txt&quot;);
   *                 myPath.isContainedIn(new Path(&quot;nohome&quot;));
   *         returns
   *                 false
   *                 myPath.isContainedIn(new Path(&quot;home&quot;));
   * // returns
   *         true
  ...
  /**
   * Checks if this path is relative
   *
   ...
   * of <code>java.util.TreeMap.subMap(...)</code>. This is useful when you need
  ...
  public String toString() {
    if (toStringCache == null) {
      toStringCache = String.join("/", elements);
    }
  ...
  public boolean contains(String element) {
    return Arrays.stream(elements).anyMatch(s -> s.equals(element));
  }

  /**
   * Returns a sub path
   *
   * @param start initial path element
   * @param end   final path element (excluded)
   * @return
   */
  public Path getSubPath(int start, int end) {
    if (start < 0 || end > elements.length) {
      throw new ArrayIndexOutOfBoundsException(String.format(
          "Arguments %d and %d are invalid since this path has %d elements",
          start, end, elements.length));
    }

    return new Path(Arrays.copyOfRange(elements, start, end));
  }
}
